...
<script type="text/javascript">
    ...
    function userEdit(id){
        var dialog=new BootstrapDialog({
            title:"用 户 编 辑",
            message:$('<div></div>').load('/operator/dialog?id='+id),
            draggable:true,
            buttons:[{
                label: '&nbsp;&nbsp;保&nbsp;&nbsp;存&nbsp;&nbsp;',
                cssClass: 'btn-primary',
                action: function(dialog) {
                    $.post('/operator/save', $('#operator-edit-form').serialize(), function (data) {
                        alert(data.message);
                        dialog.close();
                    });
                  }
    ...
    function userDel(id){
        var dialog=new BootstrapDialog({
            ...
            buttons:[{
                ...
                cssClass: 'btn-primary',
                action: function(dialog) {
                    $.post('/operator/delete',{"id":id}, function (data) {
                        alert(data.message);
                    });
    ...
    function userResetPass(id){
        var dialog=new BootstrapDialog({
            ...
            buttons:[{
                ...
                cssClass: 'btn-primary',
                action: function(dialog) {
                    $.post('/operator/resetPass',{"id":id}, function (data) {
                        alert(data.message);
                        dialog.close();
                    });
                }
    ...
    $("#btn_add").on("click",function(){
        var dialog=new BootstrapDialog({
            title:"用 户 编 辑",
            message:$('<div></div>').load('/operator/dialog?id=0'),
            draggable:true,
            buttons:[{
                label: '&nbsp;&nbsp;保&nbsp;&nbsp;存&nbsp;&nbsp;',
                cssClass: 'btn-primary',
        ...
        if(selects.length>0){
            ...
            var dialog=new BootstrapDialog({
                ...
                buttons:[{
                    ...
                    cssClass: 'btn-primary',
                    action: function(dialog) {
                        $.post('/operator/deleteRows',{"rows":ids}, function (data) {
                            alert(data.message);
                            dialog.close();
                        });
                    }
