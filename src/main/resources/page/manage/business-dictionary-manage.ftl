...
<div class="breadcrumb">
    <div class="row">
        ...
        <div class="col-sm-8">
            <div class=" panel panel-info">
                <div class="panel-heading">
                    <h3 class="panel-title">业务类型详情</h3>
                </div>
                <div class="panel-body">
                    <form id="org-edit-form" method="post" class="form-horizontal">
                        ...
                        <div class="form-group">
                            <label for="parentName" class="col-sm-2 control-label">所属类型：</label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <input type="text" id="parentName" name="parentName" readonly class="form-control form-tree" placeholder="请选择所属类型">
                       ...
                       <div class="form-group">
                            <label for="prompt" class="col-sm-2 control-label">话术提示：</label>
                            <div class="col-sm-8">
                                <textarea class="form-control" id="prompt" name="prompt"
                                       placeholder="请输入话术提示"></textarea>
                            </div>
                        </div>
                        <div class="form-group" align="center">
                            <a id="save" class="btn btn-success" href="javascript:saveRow()">保　存</a>
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <a id="cancel" class="btn btn-warning" href="javascript:rowCancel()">取　消</a>
                        </div>
                    </form>
                </div>
            </div>
...
<ul class="dropdown-menu" id="contextMenu">
    <li>
        <a href="javascript:addRow();"><i class="fa fa-plus"></i><span>添加同级</span></a>
    </li>
    <li>
        <a href="javascript:addChildRow();"><i class="fa fa-pencil-square-o"></i><span>添加下级</span></a>
    </li>
    <li>
        <a href="javascript:delRow();;"><i class="fa fa-times"></i><span>删除</span></a>
    </li>
</ul>
<script type="text/javascript">
    ...
    function query(id) {
        $.post('/businessDictionary/findById',
                {id: id},
                function (data) {
...
//        zTree.selectNode(child);
    ...
    function delRow(){
        ...
        if(node.isParent){
            ...
            $.post('/businessDictionary/delete', {id : node.id}, function (data) {
                if(data.result==1){
    ...
    function saveRow() {
        $.post('/businessDictionary/save', $('#org-edit-form').serialize(), function (data) {
            if(data.result==1){
                $.fn.zTree.init($("#tree"), setting, data.nodes);
                $.fn.zTree.init($("#tree2"), setting2, data.nodes);
                var id=$("#id").val();
